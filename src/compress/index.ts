...
import { filterCompressor, jsquashCompressor, tinypngCompressor } from '../compressor'
...
export async function compressOne(filePath: string, sourceFile?: FileDataType, options?: Options): Promise<CompressResult> {
  ...
  const compressors = filterCompressor(fileType, ...options?.compressors || [], jsquashCompressor, tinypngCompressor)
